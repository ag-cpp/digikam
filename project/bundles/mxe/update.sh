...
if [[ $DK_UPLOAD = 1 ]] ; then
    ...
    sftp -q $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64-Qt5 <<< "rm build-digikam.full.log.gz"
    sftp -q $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64-Qt5 <<< "rm build-installer.full.log.gz"
    ...
    rsync -r -v --progress -e ssh $ORIG_WD/logs/build-digikam.full.log.gz $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64-Qt5   || true
    rsync -r -v --progress -e ssh $ORIG_WD/logs/build-installer.full.log.gz $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64-Qt5 || true
