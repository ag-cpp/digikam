...
# def add_codesign_file(file):
#    hs = open("codesign_list.txt","a")
#    hs.write(file + "\n")
#    hs.close() 
...
def add_rpath(dylib):
    ...
    if not sign_binaries and install_names_cache and dylib in install_names_cache:
        return
    
    install_names.add(dylib)
    

    # add an rpath based on the level of the binary in the tree
    rpath = dylib[len(bundle_dir)+1:]
    depth = rpath.count("/")
    rpath = "@loader_path"
...
def simplify_path(new_name, dylib):
    # add code here to strip ../../ unnecessary pathing
    return new_name
...
def change_install_names(old_name, new_name, dylib):
    ...
    if (str(old_name).startswith("@") or not sign_binaries) or (install_names_cache and dylib in install_names_cache):
        return
...
def find_file(file, parent):
    ...
    for pref_dir in tmp_preferred_dirs:
        ...
        for path in paths:
        #for path in Path(pref_dir).rglob(file):
            # echo(path)
            dylib_path = os.path.dirname(path)
            if not "dSYM" in dylib_path and not "include" in dylib_path and not "bindings" in dylib_path and not "Headers" in dylib_path and os.path.isfile(path):
                # echo("Chosen:" + path)
                found_dylibs[file] = path
                return path
...
def codesign():
    for dylib in install_names:
        if install_names_cache and not dylib in install_names_cache:
...
def main(args):
    ...
    for arg in args:
        ...
        match arg_name.lower():
            ...
            case "--bundle-root":
                bundle_dir = arg_value
                preferred_dirs.append(bundle_dir)
            case "--homebrew":
                homebrew_prefix = arg_value        
                for pref_dir in preferred_dir_prefix:
                    preferred_dirs.append(os.path.join(homebrew_prefix, pref_dir))
            case "--other-libs":
                preferred_dirs.append(arg_value)
