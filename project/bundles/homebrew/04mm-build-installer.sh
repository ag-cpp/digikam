...
# opt/qt/lib/QtDBus.framework/Versions/A/QtDBus \
# opt/qt/lib/QtQuickWidgets.framework/Versions/A/QtQuickWidgets \
# "
...
for pkg in $PACKAGE_LIST ; do
    include_files="$include_files --include="**$pkg**""
done

rsync -avPm --exclude=".*" --exclude="*.h" --exclude="*.hpp" --exclude="/cache*" --exclude="*/man*" --exclude="*/homebrew*" --exclude="**cmake**" $include_files --include='*/' --exclude="*" $INSTALL_PREFIX/ $TEMPROOT/$DK_APP_CONTENTS/
...
for path in $OTHER_DIRS ; do
    ...
    if [ ! -d "$TEMPROOT/$dir" ] ; then
        echo "  Creating $TEMPROOT/$DK_APP_CONTENTS/$dir"
        mkdir -p "$TEMPROOT/$DK_APP_CONTENTS/$dir"
    fi

    echo "   Copying $INSTALL_PREFIX/$path to $TEMPROOT/$DK_APP_CONTENTS/$dir/"
    cp -aHL "$INSTALL_PREFIX/$path" "$TEMPROOT/$DK_APP_CONTENTS/$dir/"
done
...
mkdir -p "$TEMPROOT/$DK_APP_CONTENTS/Resources"
...
mkdir -p "$TEMPROOT/$DK_APP_CONTENTS/Resources/icons/"
...
touch $TEMPROOT/$DK_APP_CONTENTS/Resources/MANIFEST.txt

FILES=$(ls $ORIG_WD/data/*_manifest.txt)

for FILE in $FILES ; do
    echo $FILE
    cat $FILE >> $TEMPROOT/$DK_APP_CONTENTS/Resources/MANIFEST.txt
done
...
#for app in $INSTALL_PREFIX/$DK_APP_CONTENTS/Applications/digiKam/*.app ; do
...
echo -e "\n---------- MM Relocatable binary files"

if [ -e "processed_dylibs_cache.txt" ] ; then
    echo "---------- Removing temporary dynamic libs list processed_dylibs_cache.txt"
    rm "processed_dylibs_cache.txt"
fi

if [ -e "found_dylibs_cache.txt" ] ; then
    echo "---------- Removing temporary dynamic libs list found_dylibs_cache.txt"
    rm "found_dylibs_cache.txt"
fi

copy_lib="$INSTALL_PREFIX/bin/python3 $ORIG_WD/package_lib.py --file=$TEMPROOT/$DK_APP_CONTENTS/MacOS/digikam --bundle-root=$TEMPROOT/$DK_APP_CONTENTS --homebrew=$INSTALL_PREFIX --processed-cache=new  --found-cache=new --signed-cache=new --update-binary=true --copy=true"
eval "$copy_lib"
echo "    Signing: $TEMPROOT/digikam.app/Contents/MacOS/digikam"
codesign --force -s - $TEMPROOT/digikam.app/Contents/MacOS/digikam

copy_lib="$INSTALL_PREFIX/bin/python3 $ORIG_WD/package_lib.py --file=$TEMPROOT/showfoto.app/MacOS/showfoto --bundle-root=$TEMPROOT/$DK_APP_CONTENTS --homebrew=$INSTALL_PREFIX --processed-cache=update  --found-cache=update --signed-cache=update --update-binary=true --copy=true"
eval "$copy_lib"
echo "    Signing: $TEMPROOT/showfoto.app/Contents/MacOS/showfoto"
codesign --force -s - $TEMPROOT/showfoto.app/Contents/MacOS/showfoto
...
# DYLIBFILES=(`find $TEMPROOT/$DK_APP_CONTENTS -name "*.dylib"`)

# for FILE in $DYLIBFILES ; do
...
#         echo "     Running package_lib on: $FILE"
#         copy_lib="$INSTALL_PREFIX/bin/python3 $ORIG_WD/package_lib.py --file=$FILE --bundle-root=$TEMPROOT/$DK_APP_CONTENTS --homebrew=$INSTALL_PREFIX --cache=update --update-binary=true --copy=true"
#         eval "$copy_lib"
#     fi
# done
...
# SOFILES=(`find $TEMPROOT/$DK_APP_CONTENTS -name "*.so"`)
...
EXECFILES=(`find $TEMPROOT/$DK_APP_CONTENTS -type f ! -name "*.dylib" ! -name "*.so" -print0 | xargs -0 -n 10 file | grep "Mach-O" | cut -d ':' -f 1`)
