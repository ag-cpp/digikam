...
if [[ $DK_UPLOAD = 1 ]] ; then
    ...
    rsync -r -v --progress --bwlimit=1000 -e ssh $ORIG_WD/logs/build-digikam.full.log.gz $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64   || true
    rsync -r -v --progress --bwlimit=1000 -e ssh $ORIG_WD/logs/build-installer.full.log.gz $DK_UPLOADURL:$DK_UPLOADDIR/build.logs/win64 || true
