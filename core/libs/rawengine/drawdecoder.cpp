...
bool DRawDecoder::extractRAWDataUnprocessed(const QString& filePath,
                                            QByteArray& rawData,
                                            DRawInfo& identify,
                                            unsigned int shotSelect)
{
    ...
    unsigned short* rawBuffer = (unsigned short*) raw->imgdata.rawdata.raw_image;
    uint32 inputPlanes        = 1;

    if (!rawBuffer)
    {
        rawBuffer   = (unsigned short*)raw->imgdata.rawdata.color3_image;
        inputPlanes = 3;
    }

    if (!rawBuffer)
    {
        rawBuffer   = (unsigned short*)raw->imgdata.rawdata.color4_image;
        inputPlanes = 4;
    }

    uint32 outputPlanes = (inputPlanes == 1) ? 1 : raw->imgdata.idata.colors;

    rawData.resize((int)((int)raw->imgdata.sizes.raw_width * (int)raw->imgdata.sizes.raw_height * outputPlanes * sizeof(unsigned short)));

    unsigned short* output = reinterpret_cast<unsigned short*>(rawData.data());

    if (inputPlanes == outputPlanes)
    {
        memcpy(output, rawBuffer, (int)raw->imgdata.sizes.raw_width * (int)raw->imgdata.sizes.raw_height * outputPlanes * sizeof(unsigned short));
    }
    else
    {
        for (int i = 0 ; i < ((int)raw->imgdata.sizes.raw_width * (int)raw->imgdata.sizes.raw_height) ; ++i)
        {
            memcpy(output, rawBuffer, outputPlanes * sizeof(unsigned short));
            output    += outputPlanes;
