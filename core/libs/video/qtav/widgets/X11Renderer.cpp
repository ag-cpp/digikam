      ...
      , frame_changed(false)
    {
       ...
       //XvQueryExtension()
        ...
        if (ximg) {
            ...
            qCDebug(DIGIKAM_QTAVWIDGETS_LOG).noquote() << QString::asprintf("color mask: %X R:%1X G:%1X B:%1X", mask, ximg->red_mask, ximg->green_mask, ximg->blue_mask);
            XDestroyImage(ximg);
        }
    ...
    if (frame_orig.constBits(0)
        ...
        if (video_frame.bytesPerLine(0) == ximage->bytes_per_line) {
            ...
            if (warn_bad_pitch) {
                ...
                qCDebug(DIGIKAM_QTAVWIDGETS_LOG).noquote() << QString::asprintf("bad pitch: %d - % ximage_data[%d].size: %d", ximage->bytes_per_line, video_frame.bytesPerLine(0), index, ximage_data[index].size());
            }
