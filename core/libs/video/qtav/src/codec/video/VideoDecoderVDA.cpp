...
public:
    ...
    ~VideoDecoderVDAPrivate() {qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("~VideoDecoderVDAPrivate");}
...
VideoFrame VideoDecoderVDA::frame()
{
    ...
    if (pixfmt == VideoFormat::Format_Invalid) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("unsupported vda pixel format: %#x", CVPixelBufferGetPixelFormatType(cv_buffer));
        return VideoFrame();
    }
    ...
    public:
        ...
        virtual void* map(SurfaceType type, const VideoFormat& fmt, void* handle = 0, int plane = 0) Q_DECL_OVERRIDE {
            ...
            //qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("plane:%d, iosurface %dx%d, ctx: %p", plane, w, h, CGLGetCurrentContext());
...
#endif
    if (status) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("Failed to init VDA decoder (%#x %s): %s", status, av_err2str(status), vda_err_str(status));
        return NULL;
    }
...
bool VideoDecoderVDAPrivate::open()
{
    if (!prepare())
        return false;
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("opening VDA module");
    if (codec_ctx->codec_id != AV_CODEC_ID_H264) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("input codec (%s) isn't H264, canceling VDA decoding", avcodec_get_name(codec_ctx->codec_id));
        return false;
    }
...
void VideoDecoderVDAPrivate::close()
{
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("destroying VDA decoder");
#ifdef AV_VDA_NEW
    if (codec_ctx)
