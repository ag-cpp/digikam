...
bool VideoDecoderCedarvPrivate::open()
{
    ...
    if (format == CEDARV_STREAM_FORMAT_UNKNOW) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: codec not supported '%s'", avcodec_get_name(codec_ctx->codec_id));
        return false;
    }
    if (!cedarv) {
        int ret;
    ...
    if (cedarvRet < 0) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: set_vstream_info error: %d", cedarvRet);
        return false;
    }
...
bool VideoDecoderCedarv::decode(const Packet &packet)
{
    ...
    if (ret < 0) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: request_write failed (%d)", ret);
        return false;
    }
    ...
    if ((ret = d.cedarv->update_data(d.cedarv, &stream_data_info)) < 0) {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: update_data failed (%d)", ret);
        return false;
    }
    if ((ret = d.cedarv->decode(d.cedarv)) < 0) {
       qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: decode failed (%d)", ret);
       return false;
    }
    ret = d.cedarv->display_request(d.cedarv, &d.cedarPicture);
    if (ret > 3 || ret < 0) {
       qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("CedarV: display_request failed (%d)", ret);
