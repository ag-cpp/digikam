...
void VideoShader::initialize(QOpenGLShaderProgram *shaderProgram)
{
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("uniform locations:");
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_colorMatrix: %d", d.u_colorMatrix);
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_opacity: %d", d.u_opacity);

    if (d.u_c >= 0)
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_c: %d", d.u_c);

    if (d.u_to8 >= 0)
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_to8: %d", d.u_to8);

    if (d.u_texelSize >= 0)
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_texelSize: %d", d.u_texelSize);

    if (d.u_textureSize >= 0)
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("u_textureSize: %d", d.u_textureSize);
    ...
    if (userShaderHeader(QOpenGLShader::Vertex))
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("user uniform locations in vertex shader:");
        d.user_uniforms[VertexShader] = ParseUniforms(QByteArray(userShaderHeader(QOpenGLShader::Vertex)), shaderProgram->programId());
    }

    if (userShaderHeader(QOpenGLShader::Fragment))
    {
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("user uniform locations in fragment shader:");
...
bool VideoShader::update(VideoMaterial *material)
{
    ...
    if (mt != d.material_type || d.rebuild_program)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("Rebuild shader program requested: %d. Material type %d=>%d", d.rebuild_program, d.material_type, mt);
...
QByteArray VideoShader::shaderSourceFromFile(const QString &fileName) const
{
    ...
    {
        qCWarning(DIGIKAM_QTAV_LOG_WARN).noquote() << QString::asprintf("Can not load shader %s: %s", f.fileName().toUtf8().constData(), f.errorString().toUtf8().constData());
...
bool VideoMaterialPrivate::updateTextureParameters(const VideoFormat& fmt)
{
    ...
    qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("///////////bpp %d, bpc: %d", fmt.bytesPerPixel(), fmt.bitsPerComponent());
    ...
    for (int i = 0; i < nb_planes; ++i)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("texture width: %d - %d = pad: %d. bpp(gl): %d", texture_size[i].width(), effective_tex_width[i], pad, bpp_gl);
    ...
    if (textures.size() > nb_planes)
    {
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("try to delete %d textures", nb_delete);
        ...
        if (!textures.isEmpty())
        {
            for (int i = 0; i < nb_delete; ++i)
            {
                ...
                qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("try to delete texture[%d]: %u. can delete: %d", nb_planes+i, t, owns_texture[t]);
...
bool VideoMaterialPrivate::ensureResources()
{
            ...
            || (plane1_linesize > 0 && frame.bytesPerLine(1) != plane1_linesize))
    {
        ...
        for (int i = 0; i < nb_planes; ++i)
        {
            qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("plane linesize %d: padded = %d, effective = %d. theoretical plane size: %dx%d", i, frame.bytesPerLine(i), frame.effectiveBytesPerLine(i), frame.planeWidth(i), frame.planeHeight(i));
        ...
        qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("effective_tex_width_ratio=%f", effective_tex_width_ratio);
...
bool VideoMaterialPrivate::ensureTextures()
{
    ...
    for (int p = 0; p < nb_planes; ++p)
    {
        ...
        if (tex)
        {
            ...
            qCDebug(DIGIKAM_QTAV_LOG).noquote() << QString::asprintf("try to delete texture for plane %d (id=%u). can delete: %d", p, tex, owns_texture[tex]);
