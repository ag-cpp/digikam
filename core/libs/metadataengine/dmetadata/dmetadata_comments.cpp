...
CaptionsMap DMetadata::getItemComments(const DMetadataSettingsContainer& settings) const
{
    ...
    for (const NamespaceEntry& entry : qAsConst(settings.getReadMapping(NamespaceEntry::DM_COMMENT_CONTAINER())))
    {
        if (entry.isDisabled)
        {
            continue;
...
CaptionsMap DMetadata::getItemTitles(const DMetadataSettingsContainer& settings) const
{
    ...
    for (const NamespaceEntry& entry : qAsConst(settings.getReadMapping(NamespaceEntry::DM_TITLE_CONTAINER())))
    {
        if (entry.isDisabled)
        {
            continue;
