    ...
    : d(new Private)
{
    ...
    try
    {
        ...
        d->imageBuffer = memFile.readAll();
        memFile.close();

        if (d->imageBuffer.size() == 0)
        {
        ...
        Exiv2::Image::AutoPtr image = Exiv2::ImageFactory::open((const Exiv2::byte*)d->imageBuffer.data(), d->imageBuffer.size());
