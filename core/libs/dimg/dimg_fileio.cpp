                ...
                const DRawDecoding& rawDecodingSettings)
{
    ...
    DPluginDImg* plugin             = DImgStatic::pluginForFile(fileInfo, false);
    ...
    if (plugin && (!(loadFlags & DImgLoader::LoadPreview) || plugin->previewSupported()))
    {
        ...
        FORMAT format            = DImgStatic::loaderNameToFormat(plugin->loaderName());
    ...
    plugin = DImgStatic::pluginForFile(fileInfo, true);
...
bool DImg::save(const QString& filePath, const QString& format, DImgLoaderObserver* const observer)
{
    ...
    DPluginDImg* const plug = DImgStatic::pluginForFormat(frm);
...
DImg::FORMAT DImg::fileFormat(const QString& filePath)
{
    ...
    DPluginDImg* const plugin = DImgStatic::pluginForFile(fileInfo, false);

    if (plugin)
    {
        QString name = plugin->loaderName();
        format       = DImgStatic::loaderNameToFormat(name);
    }
