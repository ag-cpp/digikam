...
int main(int argc, char** argv)
{
    ...
    if      (info)
    {
        ...
        for (ExifToolTagInfo* it = info ; it ; it = it->next)
        {
            ...
            {
/*
                MetaEngine::ExifTagType type = meta.getExifTagType(tagNameExiv2.toLatin1().constData());
                qDebug() << tagNameExiv2 << type << data;
                meta.setExifTagData(tagNameExiv2.toLatin1().constData(), data, type);
*/

                QMetaType::Type type = meta.getExifTagVariantType(tagNameExiv2.toLatin1().constData());
                QVariant var;
                ...
                switch (type)
                {
                    ...
                    case QMetaType::QVariantList:   // Rationale as double value generated by ExifTool
                    {
                        var = data.toDouble();
                        break;
                    }
                    ...
                    case QMetaType::QByteArray:     // Undefined as string of decimal values separated by spaces
                    {
                        QByteArray conv;

                        foreach (const QString& v, QString::fromLatin1(data).split(QLatin1Char(' ')))
                        {
                            conv.append(QString::fromLatin1("%1").arg(v.toInt(), 2, 16, QLatin1Char('0')).toLatin1());
                        }
                ...
                qDebug() << tagNameExiv2 << type << data << var;
