...
void DatabaseWriter::run()
{
    while (true)
    {
        ...
        if      (package->databaseFaces.isEmpty())
        {
            ...
            if ((d->mode == FacePipeline::OverwriteUnconfirmed) && (package->processFlags & FacePipelinePackage::ProcessedByDetector))
            {
            ...
            if (!package->info.isNull() && !package->detectedFaces.isEmpty())
            {
                ...
                {
                    utils.storeThumbnails(d->thumbnailLoadThread, package->filePath,
        ...
        else
        {
            ...
            if (!package->image.isNull())
            {
                utils.storeThumbnails(d->thumbnailLoadThread, package->filePath, add.toFaceTagsIfaceList(), package->image);
            }
