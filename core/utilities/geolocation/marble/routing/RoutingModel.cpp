...
QVariant RoutingModel::headerData ( int section, Qt::Orientation orientation, int role ) const
{
    if ( orientation == Qt::Horizontal && role == Qt::DisplayRole && section == 0 ) {
        return QString::fromUtf8( "Instruction" );
    }
...
void RoutingModel::exportGpx( QIODevice *device ) const
{
    ...
    for ( int i=0; i<d->m_route.size(); ++i ) {
        ...
        content += QString::fromUtf8( "    <rtept lat=\"%1\" lon=\"%2\">\n" ).arg( lat, 0, 'f', 7 ).arg( lon, 0, 'f', 7 );
        content += QString::fromUtf8( "        <name>%1</name>\n").arg( text );
        if ( hasAltitude ) {
            content += QString::fromUtf8( "        <ele>%1</ele>\n" ).arg( maneuver.position().altitude(), 0, 'f', 2 );
        }
        content += QString::fromUtf8( "    </rtept>\n" );
    }
    ...
    for ( int i=0; i<points.size(); ++i ) {
        ...
        content += QString::fromUtf8( "      <trkpt lat=\"%1\" lon=\"%2\">\n" ).arg( lat, 0, 'f', 7 ).arg( lon, 0, 'f', 7 );
        if ( hasAltitude ) {
            content += QString::fromUtf8( "        <ele>%1</ele>\n" ).arg( point.altitude(), 0, 'f', 2 );
        }
        content += QString::fromUtf8( "      </trkpt>\n" );
    }
...
void RoutingModel::updatePosition( const GeoDataCoordinates& location, qreal speed )
{
    ...
    const qreal planetRadius = PlanetFactory::construct(QString::fromUtf8("earth")).radius();
