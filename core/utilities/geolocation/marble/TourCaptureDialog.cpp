...
void TourCaptureDialog::loadDestinationFile()
{
    ...
    if( formats.isEmpty() ) {
        QMessageBox::warning( this, i18n( "Codecs are unavailable" ), i18n( "Supported codecs are not found. (ffmpeg/avconv needs to be installed and in PATH.)" ) );
        return;
    }
...
{
    if( ui->startButton->text() == i18n("Start") ){
        ui->startButton->setText(i18n("Cancel"));
...
void TourCaptureDialog::recordNextFrame()
{
    ...
    if (!m_writingPossible) {
        ui->rate->setText(QString::fromUtf8("<font color=\"red\">%1</font>").arg(i18n("Video writing failed.")));
        return;
    }
    ...
    if (m_current_position <= duration) {
        ...
        ui->startButton->setText(i18n("Start"));
        ui->rate->setText(QString::fromUtf8("<font color=\"green\">%1</font>").arg(i18n("Video export completed.")));
        ui->closeButton->setEnabled(true);
    }
}
...
{
    ui->rate->setText(QString::fromUtf8("%1 KBytes/sec").arg(rate));
}
