...
class DIGIKAM_EXPORT AutoNavigation : public QObject
{
    Q_OBJECT
...
Q_SIGNALS:
    /**
     * signal Q_EMITted when auto center is turned on (Always re-center, re-center when required ) or off(Disabled)
     * @param recenterMode the mode for re-centering selected
     */
     void recenterModeChanged( AutoNavigation::CenterMode mode );

    /**
     * signal Q_EMITted when auto zoom is toggled
     */
